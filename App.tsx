...
import { CategoryId, Supplement, GeneratedStack } from './types';
    ...
    <div className="min-h-screen pb-32 bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex flex-1 max-w-xl gap-2">
            <div className="relative flex-1">
              <input 
                type="text" 
                placeholder="Busca por efecto o ingrediente..."
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {(sources || []).length > 0 && (
          <div className="mb-8 p-5 bg-emerald-50 border border-emerald-100 rounded-2xl animate-fade-in">
            <h4 className="text-[10px] font-black text-emerald-700 uppercase tracking-widest mb-3 flex items-center gap-2">
              <i className="fa-solid fa-book-open"></i> Referencias científicas encontradas
            </h4>
        ...
        <section className="mb-16 bg-slate-900 rounded-[2.5rem] p-8 md:p-12 text-white relative overflow-hidden shadow-2xl">
          <div className="relative z-10 max-w-2xl">
            ...
            <p className="text-slate-400 text-lg mb-8 leading-relaxed">Dime tu objetivo (ej: "angiogénesis nerviosa y foco") y la IA combinará dosis y tomas óptimas.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              <input 
                type="text" 
                placeholder="Describe tu meta física o cognitiva..."
          ...
          <i className="fa-solid fa-dna absolute right-[-50px] bottom-[-50px] text-[350px] text-white/5 rotate-12 pointer-events-none"></i>
        </section>
          ...
          {generatedStack && (
            <div className="mb-16 animate-fade-in bg-white border border-emerald-100 rounded-[2.5rem] overflow-hidden shadow-2xl">
              <div className="bg-emerald-600 p-10 text-white flex justify-between items-start">
                <div>
                  <h3 className="text-3xl font-black mb-2">{generatedStack.title}</h3>
                  <p className="text-emerald-50 opacity-90">{generatedStack.description}</p>
                </div>
                <button onClick={() => setGeneratedStack(null)} className="p-2 hover:bg-black/10 rounded-full transition-all"><i className="fa-solid fa-times text-xl"></i></button>
              </div>
                ...
                {generatedStack.items.map((item, i) => (
                  <div key={i} className="bg-slate-50 p-6 rounded-3xl border border-slate-100">
                    <h4 className="font-black text-slate-900 text-xl mb-4 flex items-center gap-3">
                      <span className="w-8 h-8 bg-emerald-100 text-emerald-700 rounded-lg flex items-center justify-center text-sm">{i+1}</span>
                      {item.supplement}
                    </h4>
                    <div className="space-y-3 text-sm">
                      <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-slate-400 font-bold">DOSIS</span><span className="font-black text-emerald-700">{item.dosage}</span></div>
                      <div className="flex justify-between border-b border-slate-200 pb-2"><span className="text-slate-400 font-bold">TOMA</span><span className="font-black text-slate-700">{item.timing}</span></div>
                      <p className="text-slate-600 italic mt-3">"{item.reason}"</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <section>
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-3xl font-black text-slate-900 tracking-tight">
              {isSearchingAI ? 'Resultados de Investigación' : 'Directorio de Suplementos'}
              <span className="ml-4 text-xs font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">{filteredSupplements.length} ítems</span>
            </h2>
            {selectedCategory && (
              <div className="flex gap-2">
                {CATEGORIES.find(c => c.id === selectedCategory)?.subcategories.map(sub => (
                  <button 
                    ...
                    className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase transition-all ${selectedSubcategory === sub ? 'bg-slate-800 text-white shadow-lg' : 'bg-white text-slate-400 border border-slate-200 hover:border-emerald-300'}`}
                  >
                    {sub}
                  </button>
                ))}
          ...
          {filteredSupplements.length === 0 && (
            <div className="bg-white rounded-[3rem] p-20 text-center border-2 border-dashed border-slate-200 animate-fade-in">
              <i className="fa-solid fa-magnifying-glass-plus text-5xl text-slate-200 mb-6"></i>
              <h3 className="text-2xl font-black text-slate-800 mb-2">No encontrado localmente</h3>
              <p className="text-slate-500 mb-8">Nuestra base local no tiene este dato, ¿quieres que la IA investigue en tiempo real?</p>
              <button 
                onClick={() => handleAISearch()}
                className="bg-slate-900 text-white px-10 py-4 rounded-2xl font-black tracking-widest uppercase text-xs hover:bg-emerald-600 transition-all shadow-xl"
              >
      ...
      <footer className="fixed bottom-0 w-full bg-slate-900 text-white p-4 border-t border-white/5 backdrop-blur-md z-[60]">
        <div className="max-w-7xl mx-auto flex justify-between items-center px-4">
          <p className="text-[10px] font-bold text-slate-400 max-w-lg leading-tight uppercase tracking-wider">
            ⚠️ Información educativa. Esta plataforma sintetiza literatura científica pero <strong className="text-white">no sustituye consejo médico profesional.</strong>
          </p>
          <div className="flex items-center gap-4">
            <span className="text-[9px] font-black text-emerald-400 bg-emerald-400/10 px-3 py-1 rounded-full border border-emerald-400/20 uppercase tracking-widest">Grounding v2.5 Active</span>
          </div>
        </div>
      </footer>
    </div>
  ...
  return (
    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all overflow-hidden flex flex-col group h-full border-b-4 hover:border-b-emerald-500">
      <div className="p-8 flex-1">
        <div className="flex justify-between items-start mb-6">
          <div className="flex gap-2 flex-wrap">
            ...
            <span className={`px-3 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-widest border ${supplement.source === 'ai' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' : 'bg-slate-50 text-slate-500 border-slate-100'}`}>
              <i className={`fa-solid ${supplement.source === 'ai' ? 'fa-wand-sparkles' : 'fa-database'} mr-1.5`}></i>
              {supplement.source === 'ai' ? 'AI Research' : 'Local Base'}
            </span>
          </div>
            ...
            {positiveEffects.length > MAX_VISIBLE && (
              <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-200/50">
                <div className="flex items-center gap-3">
                  <button onClick={prev} className="w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-200 flex items-center justify-center transition-all shadow-sm">
                    <i className="fa-solid fa-arrow-left text-[10px]"></i>
                  </button>
                  <div className="flex gap-1.5">
                    {Array.from({ length: totalPages }).map((_, i) => (
                      <div key={i} className={`h-1.5 rounded-full transition-all ${i === benefitIndex ? 'w-4 bg-emerald-500 shadow-sm' : 'w-1.5 bg-slate-200'}`} />
                    ))}
                  </div>
                  <button onClick={next} className="w-8 h-8 rounded-full bg-white border border-slate-200 text-slate-400 hover:text-emerald-600 hover:border-emerald-200 flex items-center justify-center transition-all shadow-sm">
            ...
            <div className="animate-fade-in pt-6 space-y-6 border-t border-slate-100">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                  <span className="block text-[8px] font-black text-slate-400 uppercase mb-1">Dosis Mínima</span>
                  <span className="text-xs font-black text-slate-900">{supplement.minDose}</span>
                </div>
                <div className="bg-emerald-50 p-4 rounded-2xl border border-emerald-100">
                  <span className="block text-[8px] font-black text-emerald-600 uppercase mb-1">Dosis Ideal</span>
              ...
              {supplement.timing && (
                <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100 flex items-center gap-4">
                  <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center text-blue-600 shadow-sm border border-blue-100"><i className="fa-regular fa-clock"></i></div>
                  <div>
                    <span className="block text-[8px] font-black text-blue-600 uppercase mb-0.5">Protocolo Sugerido</span>
                    <span className="text-xs font-black text-slate-900">{supplement.timing}</span>
                  </div>
                </div>
              )}
            </div>
      ...
      <button onClick={() => setExpanded(!expanded)} className="w-full py-5 bg-slate-50 text-[10px] font-black text-slate-400 uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all flex items-center justify-center gap-3">
        {expanded ? 'Contraer Investigación' : 'Ver Detalles Completos'}
